...
<html>
    <head>
        <title>Saakshi Stock Management</title>
...
$text="";
$new_array=$_REQUEST['text'];	
$arr=explode('@',$new_array);

foreach ($arr as $val) {


$arr_quan=explode('$',$val);
  

	for($tt=1;$tt<=$arr_quan[1];$tt++) {
...
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>&id=12" method="post">
...
?>
                    ...
                    <tr style="display: none;">
                        ...
                        <td><?php echo getSelectHtml('font_family', $font_family, listfonts('../font')); ?> <?php echo getInputTextHtml('font_size', $font_size, array('type' => 'number', 'min' => 1, 'max' => 300)); ?></td>
                    </tr>
					<!--<tr style="display: block; width: 50%px;">
                        <td><label for="scale">Number of Code</label></td>
                        <td><?php echo getInputTextHtml('ncode1', $ncode1, array('type' => 'number', 'min' => 1, 'max' => 100, 'required' => 'required')); ?></td>
                         <td><?php echo getInputTextHtml('ncode2', $ncode2, array('type' => 'number', 'min' => 1, 'max' => 100, 'required' => 'required')); ?></td>
                        <td> <div class="generate" style="float: left"><input type="submit" value="Generate" /></div></td>
                    </tr>-->
